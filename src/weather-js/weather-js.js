...
function mainWeather(weatherLat, weatherLong) {
	...
	$.getJSON(url + apiKey + "/" + weatherLat + "," + weatherLong + "?units=uk&lang=ru&callback=?", function (data) {
		...
		for (var i = 0; i < 24; i++) {
			...
			$(".sumhr1").append(
				'<div class="' +
					...
					"wind:" +
					"</p>" +
					'<i class="fas fa-long-arrow-alt-up" style="transform: rotate(' +
					data.hourly.data[i].windBearing +
					'deg)"></i>' +
					"<b>" +
					data.hourly.data[i].windSpeed.toFixed(1) +
					"</b>" +
					"<p>" +
					"M/s" +
					"</p>" +
		...
		for (i = 0; i < 7; i++) {
			...
			var dailyapparentTemperatureHighTime = moment
				.unix(data.daily.data[i].apparentTemperatureHighTime)
				.tz(data.timezone)
				.format("HH:mm");
			var dailyapparentTemperatureLowTime = moment
				.unix(data.daily.data[i].apparentTemperatureLowTime)
				.tz(data.timezone)
				.format("HH:mm");
			...
			$(".daily").append(
				'<article><div class="row1">' +
					...
					" - " +
					dayprecipIntensityMaxTime +
					" - " +
					dayprecipType +
					"</b>" +
					"</div>" +
					'<div class="row2" style="background-image: linear-gradient(to right,#006ac0 ' +
					(dailysunrisePercent - 1) +
					"%," +
					"#c07600 " +
					dailysunrisePercent +
					"%," +
					...
					" - " +
					dailytemperatureHighTime +
					"</b>" +
					// '<b> - ' + '</b>' +
					"<p>tempLow</p><b>" +
					dailytemperatureLow +
					" - " +
					dailytemperatureLowTime +
					"</b>" +
					// '<b> - ' +'</b>' +
					"</div>" +
					"<div>" +
